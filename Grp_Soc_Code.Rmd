...
```{r}
# VISUALIZATION
ggplot(data=grpyr_df, aes(x=rain_div, y=pct_feeding)) + geom_point()+labs(x = "Rainfall Diversity", y="Proportion of Time Spent Feeding", title = "Relationship of rainfall diversity vs. proportion of time spent feeding")+geom_smooth(method=lm, se=FALSE, col='blue')

ggplot(data=grpyr_df, aes(x=rain_div, y=pct_moving)) + geom_point()+labs(x = "Rainfall Diversity", y="Proportion of Time Spent Moving", title = "Relationship of  rainfall diversity vs. proportion of time spent feeding")+geom_smooth(method=lm, se=FALSE, col='blue')

ggplot(data=grpyr_df, aes(x=rain_div, y=pct_relative)) + geom_point()+labs(x = "Rainfall Diversity", y="Proportion of Time Spent Feeding Rel to Moving", title = "Relationship of  rainfall diversity vs. proportion of time spent feeding relative to moving")+geom_smooth(method=lm, se=FALSE, col='blue')
```
...
grpyr_df<-grpyr_df%>%
  rowwise()%>%
  mutate(foodtype=ifelse(permanent>as.Date('1999-01-01'),list(count_foodcode_new(sub_members_df)),list(count_foodcode_old(sub_members_df))))


## NEED DEBUGGING
grpyr_df<-grpyr_df%>%unnest(foodtype)
```
...
```{r}
logis.model.feeding<-glmer(cbind(n_feeding, (n_total_sample-n_feeding)) ~ scale(n_adults)+scale(net_density_wog)+ (1|gid)+(1|hydroyear), data=grpyr_df,family = "binomial"(link="logit"))
#logistic regression
